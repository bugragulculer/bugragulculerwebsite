  ...
  switch (type) {
    case SIGNIN:
      try {
        ...
        yield initRequest();
        const snapshot = yield call(getUser, user.user.uid);
        if (snapshot.data()) {
    ...
    case SIGNUP:
      try {
        ...
        const ref = yield call(createAccount, payload.email, payload.password);
    ...
    case ON_AUTHSTATE_SUCCESS: {
      const snapshot = yield call(getUser, payload.uid);
      ...
      if (snapshot.data()) {
        ...
        const user = {
          ...
          //avatar: payload.photoURL ? payload.photoURL : logo,
          //banner: logo,
          email: payload.email,
          address: "",
          basket: [],
